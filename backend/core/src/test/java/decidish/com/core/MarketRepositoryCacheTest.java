...
class MarketRepositoryCacheTest {
    ...
    @Test
    void testCache_findByIdWithProducts() {
        Long marketId = 3L; // Ensure this exists in your @BeforeEach setup

        // 1. First Call - Should hit DB (1 Query expected)
        // Note: Because of JOIN FETCH, this generates 1 query that gets Market + Products
        Optional<Market> result1 = marketRepository.findByIdWithProducts(marketId);
        assertThat(hibernateStats.getQueryExecutionCount()).isEqualTo(1);
        assertThat(result1).isPresent();
        assertThat(result1.get().getProducts()).isNotEmpty(); // Verify products loaded

        // 2. Second Call - Should hit Cache (DB count remains 1)
        Optional<Market> result2 = marketRepository.findByIdWithProducts(marketId);
        assertThat(hibernateStats.getQueryExecutionCount()).isEqualTo(1);
        
        // 3. Verify Cache Internals
        var cache = cacheManager.getCache("markets_id_with_products");
        assertThat(cache.get(marketId)).isNotNull();
    }

    @Test
    void testNoCache_findByIdWithProducts_WhenUnlessMet() {
        // Your condition is: unless = "#a0==2L" (Argument 0 equals 2L)
        Long marketId = 2L; 

        // 1. First Call - Hits DB
        marketRepository.findByIdWithProducts(marketId);
        assertThat(hibernateStats.getQueryExecutionCount()).isEqualTo(1);

        // 2. Second Call - Hits DB AGAIN
        marketRepository.findByIdWithProducts(marketId);
        assertThat(hibernateStats.getQueryExecutionCount()).isEqualTo(2);

        // 3. Verify it is NOT in cache
        var cache = cacheManager.getCache("markets_id_with_products");
        assertThat(cache.get(marketId)).isNull();
    }

    @Test
    void testCache_findProductByMarketAndId() {
        // Assuming Market 3 has Product 100 created in setup()
        Long marketId = 3L;
        Long productId = 100L;

        // 1. First Call - Hits DB
        Optional<Product> p1 = marketRepository.findProductByMarketAndId(marketId, productId);
        assertThat(hibernateStats.getQueryExecutionCount()).isEqualTo(1);

        // 2. Second Call - Hits Cache
        Optional<Product> p2 = marketRepository.findProductByMarketAndId(marketId, productId);
        assertThat(hibernateStats.getQueryExecutionCount()).isEqualTo(1);

        // 3. Verify Custom Key Generation
        // Your key is: "#marketId + '-' + #productId"
        String expectedKey = "3-100";
        var cache = cacheManager.getCache("market_products");
