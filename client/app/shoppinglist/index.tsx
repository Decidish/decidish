                        ...
                        <View className='flex-1'>
                            <View className='bg-white rounded-2xl shadow-sm p-6'>
                                <View className='flex-row justify-between items-center mb-6'>
                                    ...
                                    <View className='flex-row bg-gray-100 rounded-full p-1'>
                                        ...
                                        >
                                            <Text
                                                className={`text-sm font-medium ${viewMode === 'category' ? 'text-gray-900' : 'text-gray-600'}`}>
                                                By Category
                                            </Text>
                                        </TouchableOpacity>
                                        ...
                                        >
                                            <Text
                                                className={`text-sm font-medium ${viewMode === 'all' ? 'text-gray-900' : 'text-gray-600'}`}>
                                                All Items
                                            </Text>
                                        </TouchableOpacity>
                                    </View>
                                ...
                                {viewMode === 'category' ? (
                                    ...
                                    Object.entries(groupedItems).map(([category, catItems]) => {
                                        ...
                                        return (
                                            <View key={category}
                                                  className='mb-6 border-2 border-gray-100 rounded-xl overflow-hidden'>
                                                {/* Category Header */}
                                                <TouchableOpacity
                                                ...
                                                >
                                                    <View className='flex-row items-center gap-3'>
                                                        <View
                                                            className={`w-10 h-10 rounded-full items-center justify-center ${getCategoryColor(category)}`}>
                                                            <Text
                                                                className='font-bold text-lg'>{getCategoryIcon(category)}</Text>
                                                        </View>
                                                        <View>
                                                            <Text
                                                                className='font-semibold text-gray-900'>{category}</Text>
                                                            <Text
                                                                className='text-xs text-gray-500'>{checkedCount} / {catItems.length} items</Text>
                                                        </View>
                                                    </View>
                                                    {isCollapsed ? <ChevronDown size={20} color="#9ca3af"/> :
                            ...
                            {deletedItems.length > 0 && (
                                <View className='mt-6 bg-gray-50 rounded-2xl p-4 border border-gray-200'>
                                    ...
                                    >
                                        <Text className='font-bold text-gray-600'>Deleted Items
                                            ({deletedItems.length})</Text>
                                        {showDeleted ? <ChevronUp size={18} color="#666"/> :
                                    ...
                                    {showDeleted && (
                                        <View className='mt-4 gap-2'>
                                            {deletedItems.map(item => (
                                                <View key={item.id}
                                                      className='flex-row justify-between items-center bg-white p-3 rounded-lg opacity-60'>
                                ...
                                <View className='gap-3 relative'>
                                    ...
                                    <View className="relative z-50">
                                        ...
                                        >
                                            ...
                                            {isCatOpen ? <ChevronUp size={20} color="#6b7280"/> :
                                        ...
                                        {isCatOpen && (
                                            <View
                                                className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-50 max-h-48">
                                                <ScrollView nestedScrollEnabled>
                                                    {categories.map((cat) => (
                                                        ...
                                                        >
                                                            <Text
                                                                className={`text-sm ${customCategory === cat ? 'font-bold text-teal-600' : 'text-gray-700'}`}>
                                                                {cat}
                                                            </Text>
                                                        </TouchableOpacity>
                                                    ))}
                                ...
                                <View className='gap-3 w-full'>
                                    {recipes.map(recipe => (
                                        <View key={recipe.title}
                                              className='flex-row w-full items-center gap-3 p-2 bg-gray-50 rounded-lg'>
                                            <Image
